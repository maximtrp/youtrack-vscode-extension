...
export class SprintsIssuesProvider
  implements
    vscode.TreeDataProvider<IssueItem | SprintItem | GroupingItem | None>
{
        ...
        if (element.contextValue === "sprint" && groupby !== "None") {
          ...
          try {
            ...
            if (issues && issues.length > 0) {
              return issues.map(
                (issue) => new IssueItem(issue, this.client?.self)
              )
            } else {
  ...
  async addIssue() {
    if (this.client && this.project) {
      ...
      try {
        ...
        vscode.window.showErrorMessage(
          `Issue was not created due to the error: ${(error as Error).message}`
        )
        return
      }
      ...
      if (this.agile && this.sprints) {
        ...
        if (selectedSprint) {
          ...
          try {
            await this.client.addIssueToSprint(
              this.agile.id,
              selectedSprintId,
              createdIssue.id
            )
      ...
      vscode.window.showInformationMessage(
        "Please select a project to add an issue"
      )
    }
  }
  ...
  async updateIssueAssignee(item: IssueItem) {
    if (this.client) {
      ...
      if (!users || users.length === 0) {
        vscode.window.showInformationMessage(
          `No users found to assign this issue to`
        )
        return
      }

      const selectedUserInfo: string | undefined =
      ...
      const selectedUser = users.find(
        (user) => user.login === selectedUserLogin
      )
      if (!selectedUser) {
      ...
      try {
        ...
        vscode.window.showErrorMessage(
          `Issue assignee was not updated due to this error: ${e}`
        )
      }
    }
  }
    ...
    if (this.client) {
      ...
      try {
        ...
        vscode.window.showWarningMessage(
          `Issue was not updated due to this error: ${e}`
        )
      }
    }
  }
    ...
    if (this.client) {
      ...
      if (confirm) {
        try {
          ...
          vscode.window.showWarningMessage(
            `Issue was not deleted due to the error: ${(error as Error).message}`
          )
        }
      }
    }
  ...
  async createBranch(item: IssueItem) {
    try {
      ...
      vscode.window.showInformationMessage(
        `Error occurred while creating a git branch: ${(error as Error).message}`
      )
    }
  }
