...
export class IssueItem extends vscode.TreeItem {
  ...
  constructor(issue: Issue, self?: User) {
    ...
    let assignee: User = (
      issue.customFields?.find((field) => field.name === "Assignee") || {}
    ).value
  ...
  prepareDescription(item: IssueItem): string {
    ...
    let reporter = item.issue.reporter
      ? `Created by ${item.issue.reporter.fullName}`
      : ""
    let reportDate = item.issue.created
      ? ` on ${fmt.format(item.issue.created)}`
      : ""
    description += `<p>${reporter}${reportDate}</p>`
    let updater = item.issue.updater
      ? `Updated by ${item.issue.updater.fullName}`
      : ""
    let upDate = item.issue.updated
      ? ` on ${fmt.format(item.issue.updated)}`
      : ""
